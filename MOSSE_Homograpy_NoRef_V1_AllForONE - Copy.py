...
def print_data_img(img):
    
    offset = 30
    cv2.putText(img, "RHO - Matching Using Mask ", (int((img.shape[1] - len("RHO - Matching Using Mask "))/2),10),   cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0,0,255),2)
    cv2.putText(img, "Detector = " + detector_type , (10,offset), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (50,170,50),2)
    cv2.putText(img, "Descriptor = " + descriptor_type , (10,2*offset), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (50,170,50),2)
    cv2.putText(img,"Inliers Percent = " + str(GOOD_MATCH_PERCENT), (10,3*offset), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (50,170,50),2)
    cv2.putText(img,"Threshold = " + str(threshold), (10,4*offset), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (50,170,50),2)
    cv2.putText(img,"Matching = " + Match_methods , (10,5*offset), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (50,170,50),2)
    cv2.putText(img_ply, "FPS : " + str(int(fps)), (10,6*offset), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (50,170,50), 2)
...
#                        print("Number of key points ", len(kp_prev))
                    ...
                    if(Homography_model is not None):
                        ...
                        if(flag_show == True):
                               p1 = (int(bbox[0]), int(bbox[1]))
                               p2 = (int(bbox[0] + bbox[2]), int(bbox[1] + bbox[3]))
                               cv2.rectangle(img_ply, p1, p2, (255,255,0), 2, 1)
                   
                        
                        img_ply = print_data_img(img_ply)
